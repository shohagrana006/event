...
class AppServices {
    ...
    public function sendOrderConfirmationEmail($order, $emailTo,$orderReference,$services) {
        ...
        $email = (new \Swift_Message($this->translator->trans('Your tickets bought from') . ' ' . $this->getSetting('website_name')))
                ...
                ->setTo($emailTo)
                ->setBody(
                        $this->templating->render('Dashboard/Shared/Order/confirmation-email.html.twig', ['order' => $order , 'description' => $description ]), 'text/html')
    ...
    public function getChatbots() {
        ...
        $userId = $user->getId();
        $sql = "SELECT * FROM chatbot_lists WHERE org_id = :userId";
        $statement = $this->em->getConnection()->prepare($sql);
        $statement->execute(['userId' => $userId]);
        $chatbotLists = $statement->fetchAll();
        return $chatbotLists;
    }

    public function getChatbotsAsChoices(): array
    {
        $chatbotLists = $this->getChatbots();
        $choices = [];
        foreach ($chatbotLists as $chatbot) {
            $choices[$chatbot['custom_name']] =  $chatbot['id'];
        }
        return $choices;
    }
    ...
    public function getOrders($criterias) {
        ...
        // dd($status, $user, $organizer, $event, $eventDate, $eventTicket, $reference, $upcomingtickets, $datefrom, $dateto, $paymentgateway, $sort, $order, $limit, $count, $ordersQuantityByDateStat, $sumOrderElements);
