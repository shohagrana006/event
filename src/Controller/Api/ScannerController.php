...
class ScannerController extends Controller {
    ...
    public function login(Request $request, AppServices $services, UserPasswordEncoderInterface $passwordEncoder, Packages $assetsManager, TranslatorInterface $translator): Response {
        ...
        if (!$user) {
            return $this->json(array("type" => "error", "message" => $translator->trans("dInvalid credentials")));
        }
        if (!$passwordEncoder->isPasswordValid($user, $password)) {
            return $this->json(array("type" => "error", "message" => $translator->trans("qInvalid credentials")));
        }
        if (!$user->isEnabled()) {
